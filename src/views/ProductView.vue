...
const formValidation = () => {
  const requiredFields = ['title', 'description', 'price', 'category', 'subcategory', 'brandId', 'stock', 'rating'];
  let isValid = true;
...
const onSubmitForm = async () => {
  ...
  try {
    ...
    if (product.value.image instanceof File) {
      console.log("product.value.image ", product.value.image)
      formData.append('image', product.value.image);
    }

    formData.append('category', product.value.category?._id || '');
    formData.append('subcategory', product.value.subcategory?._id || '');
    formData.append('brand', product.value.brandId?._id || '');
    formData.append('stock', product.value.stock);
    formData.append('rating', String(product.value.rating));

    if (updatedProductId.value) {
      await api.updateProduct(updatedProductId.value, formData as Product);
    } else {
